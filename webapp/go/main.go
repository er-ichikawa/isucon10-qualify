...
var mySQLConnectionData2 *MySQLConnectionEnv2
...
type MySQLConnectionEnv2 struct {
	Host     string
	Port     string
	User     string
	DBName   string
...
func NewMySQLConnectionEnv2() *MySQLConnectionEnv2 {
	return &MySQLConnectionEnv2{
...
//ConnectDB2 isuumoデータベースに接続する
func (mc *MySQLConnectionEnv2) ConnectDB2() (*sqlx.DB, error) {
	dsn := fmt.Sprintf("%v:%v@tcp(%v:%v)/%v", mc.User, mc.Password, mc.Host, mc.Port, mc.DBName)
	return sqlx.Open("mysql", dsn)
}
...
func main() {
	...
	db2, err = mySQLConnectionData2.ConnectDB2()
	if err != nil {
		e.Logger.Fatalf("DB connection failed : %v", err)
	}
...
func postChair(c echo.Context) error {
    ...
    for i, row := range records {
        ...
        query += fmt.Sprintf(`(%d,"%s","%s","%s","%d",%d,%d,%d,"%s","%s","%s",%d,%d)`, id, name, description, thumbnail, price, height, width, depth, color, features, kind, popularity, stock)
        if recordLen-1 > i {
            query += ","
        }
    }
...
func searchEstateNazotte(c echo.Context) error {
	...
	estates := []Estate{}
	err = db2.Select(&estates, query, b.BottomRightCorner.Latitude, b.TopLeftCorner.Latitude, b.BottomRightCorner.Longitude, b.TopLeftCorner.Longitude)
	if err == sql.ErrNoRows {
...
func searchEstateNazotte(c echo.Context) error {
	...
	query := fmt.Sprintf(`SELECT id,thumbnail,name,description,latitude,longitude,address,rent,door_height,door_width,features,popularity FROM estate WHERE latitude <= ? AND latitude >= ? AND longitude <= ? AND longitude >= ? AND ST_Contains(ST_PolygonFromText(%s), geo) ORDER BY popularity DESC, id ASC`, coordinates.coordinatesToText())
	c.Echo().Logger.Infof("query : %v", query)
	...
	query := fmt.Sprintf(`SELECT * FROM estate WHERE ST_Contains(ST_PolygonFromText(%s), point) ORDER BY popularity_desc, id`, coordinates.coordinatesToText())
	c.Echo().Logger.Errorf("query: %v", query)
	err = db.Get(&validatedEstate, query)
	if err != nil {
		if err == sql.ErrNoRows {
			c.Echo().Logger.Errorf(" sql.ErrNoRows: %v", err)
			return c.NoContent(http.StatusInternalServerError)
		} else {
